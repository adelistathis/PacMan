...
def show_go_screen():
    ...
    The menu pops up when Player starts the game and after they win or lose.
    """
    screen.fill(BLACK)
...
while running:
    if game_over:
        ...
        # Create the walls
        wall_list = ExtraFunctions.setup_room()
        all_sprites_list.add(wall_list)

        # Create the blocks
        block_list = ExtraFunctions.create_blocks(wall_list)
        ...
        # create a list of the Player's lives and add each Life() object to the list
    ...
    if pac.die(ghost_list): # will run the function in order to check the if-statement
        ExtraFunctions.remove_life(life_list)
    if not pac.hasLives():
        game_over = True
